...
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>PIA Participant Workspace</title><style>{_shared_style()}
.participant-shell{{width:min(1440px,calc(100% - 32px));}}
.participant-header{{display:flex;justify-content:space-between;align-items:start;gap:18px}}
.participant-header h1{{font-size:clamp(2.5rem,5vw,4.4rem);max-width:900px}}
.participant-resume{{margin-top:20px}}
.participant-resume-head{{display:flex;align-items:center;justify-content:space-between;gap:18px;flex-wrap:wrap}}
.participant-resume-head h2{{margin-bottom:4px}}
.participant-resume .session-summary{{display:grid;grid-template-columns:minmax(220px,1fr) minmax(260px,2fr) auto;align-items:center;gap:14px}}
.participant-resume .session-summary button{{margin:0}}
.participant-setup{{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:12px;align-items:end}}
.participant-setup .wide{{grid-column:span 2}}.participant-setup label{{margin-top:0}}
.participant-upload{{display:grid;grid-template-columns:auto minmax(220px,1fr) auto;gap:10px;align-items:center;margin-top:16px}}
.participant-review-grid{{display:grid;grid-template-columns:250px minmax(360px,1fr) 360px;gap:16px;margin-top:20px}}
.participant-review-grid>*{{min-width:0}}.participant-pane{{border:1px solid var(--line);border-radius:16px;background:#fff;padding:16px;min-height:460px;min-width:0;overflow:hidden}}
.participant-pane h2{{font:700 1.15rem/1.2 Inter,sans-serif}}
.participant-pane p{{max-width:100%;overflow-wrap:anywhere;word-break:break-word}}
.participant-document,.participant-statement{{width:100%;text-align:left;color:var(--ink);background:#fff;border:1px solid var(--line);border-radius:10px;margin:8px 0;padding:12px;cursor:pointer}}
.participant-document.active,.participant-statement.active{{border-color:var(--sage);background:var(--pale)}}
.participant-document small,.participant-statement small{{display:block;color:var(--muted);margin-top:5px}}
.participant-statement p{{display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;margin:0;overflow-wrap:anywhere}}
@media(max-width:950px){{.participant-review-grid{{grid-template-columns:1fr}}.participant-pane{{min-height:0}}.participant-setup{{grid-template-columns:1fr 1fr}}}}
@media(max-width:760px){{.participant-header{{display:block}}.participant-header .identity{{margin-top:12px}}.participant-resume .session-summary,.participant-setup,.participant-upload{{grid-template-columns:1fr}}.participant-setup .wide{{grid-column:span 1}}}}
</style></head>
<body class="participant-theme"><main class="participant-shell">
  <div class="participant-header"><div><div class="eyebrow">PIA · Participant workspace</div>
  ...
  <section class="notice"><strong>Protection controls are active.</strong>
    <div class="controls"><div class="control"><strong>Encrypted at rest</strong><span>Protected with per-session keys.</span></div>
    <div class="control"><strong>Inspected in memory</strong><span>Document bytes are checked before encrypted storage.</span></div>
    <div class="control"><strong>Finite lifecycle</strong><span>You control submission, withdrawal, and retention.</span></div></div>
  </section>
  <section class="card participant-resume"><div class="participant-resume-head"><div>
    <div class="eyebrow">Continue saved work</div><h2>Pick up where you left off.</h2>
    <p class="quiet">Only active workspaces associated with this participant account are shown.</p></div>
    <button id="participantLoadSessions" class="secondary" type="button">Show my saved workspaces</button></div>
    <div id="participantSavedSessions"></div>
  </section>
  ...
  <section class="card"><div class="eyebrow">Workspace</div>
    <div class="participant-setup">
    <div><label for="participantLabel">Private workspace label</label><input id="participantLabel" placeholder="My professional review"></div>
    <div class="wide"><label for="participantPurpose">What would you like help understanding?</label><input id="participantPurpose" placeholder="The capabilities my experience demonstrates"></div>
    <div><label for="participantScope">Output</label><select id="participantScope"><option value="evidence_report">Evidence review and report</option><option value="evidence_only">Evidence review only</option></select></div>
    <div><label for="participantRetention">Retention</label><select id="participantRetention"><option value="14_days">14 days</option></select></div>
    <div class="check wide"><input id="participantConsent" type="checkbox"><label for="participantConsent">I understand this is private local work and I may withdraw.</label></div>
    <button id="participantSave">Open private workspace</button></div>
    <div id="participantSessionStatus" class="status">Open or resume a private workspace to begin.</div>
    ...
    <button id="participantStage" class="secondary" disabled>Save selected document</button></div><div id="participantArtifactStatus" class="status">Create your private workspace before adding documents.</div>
  </section>
  <section class="participant-review-grid" aria-label="Participant evidence review">
    <aside class="participant-pane"><h2>Documents</h2><div id="participantDocuments"><p class="quiet">No workspace selected.</p></div></aside>
    <section class="participant-pane"><h2>Extracted statements</h2><div id="participantArtifactResults"><p class="quiet">Select a document to prepare its evidence.</p></div></section>
    <aside class="participant-pane"><h2>Selected statement</h2><div id="participantSelectedEvidence"><p class="quiet">Select a statement to keep or exclude it.</p></div></aside>
  </section>
  <section class="card"><div class="eyebrow">4 · Your report</div>
    <h2>Review what is ready to share.</h2>
    <p class="quiet">Evidence acceptance confirms what the source says. A capability report requires a separate bounded link from that accepted evidence to a capability. The report appears here after that capability mapping is accepted and passes output assurance.</p>
    ...
    <button id="participantSubmit" type="button" disabled>Submit reviewed evidence</button>
    <div id="participantSubmitStatus" class="status">Review the evidence summary before submitting it for authorized review.</div>
  </section>
...
let participantSessionId="",participantProcessingState="";
async function participantRequest(path,body=null,method="POST"){{const options={{method,headers:{{"X-PIA-CSRF":participantCsrf}}}};if(body!==null){{options.headers["Content-Type"]="application/json";options.body=JSON.stringify(body);}}const response=await fetch(path,options);const data=await response.json();if(!response.ok)throw new Error(data.error||"Request failed.");return data;}}
...
async function participantResumeSession(sessionId){{const result=await participantRequest("/api/sessions/resume",{{intake_session_id:sessionId}});participantSessionId=result.session.intake_session_id;participantProcessingState=result.session.processing_state||"";document.getElementById("participantLabel").value=result.session.participant_label||"";document.getElementById("participantPurpose").value=result.session.purpose||"";document.getElementById("participantConsent").checked=true;document.getElementById("participantSave").disabled=true;document.getElementById("participantFile").disabled=false;document.getElementById("participantDocumentType").disabled=false;document.getElementById("participantDocuments").replaceChildren();document.getElementById("participantArtifactResults").replaceChildren();const savedExtractions=new Map((result.evidence_extractions||[]).map(item=>[item.source_artifact_id,item]));(result.artifacts||[]).forEach(artifact=>participantArtifactCard(artifact,savedExtractions.get(artifact.source_artifact_id)));const completed=participantProcessingState==="review_complete";participantStatus(completed?"Authorized evidence review is complete. Check the report stage below.":"Saved workspace restored. Continue with your documents and evidence review.");document.getElementById("participantReport").textContent=completed?"Check capability report":"View participant overview";document.getElementById("participantReport").disabled=false;document.getElementById("participantReportStatus").textContent=completed?"Evidence review is complete. Check whether capability mapping and report assurance are ready.":"Your report will become available after evidence review is complete.";document.getElementById("participantArtifactStatus").textContent=(result.artifacts||[]).length+" saved document(s) restored. Select a document to continue.";}}
async function participantLoadSessions(){{const target=document.getElementById("participantSavedSessions");target.replaceChildren();try{{const result=await participantRequest("/api/sessions",null,"GET");if(!result.sessions.length){{target.textContent="No active saved workspaces were found for this participant account.";return;}}result.sessions.forEach(session=>{{const row=document.createElement("div");row.className="session-summary";const title=document.createElement("strong");title.textContent=session.participant_label;const detail=document.createElement("div");detail.className="quiet";detail.textContent=session.artifact_count+" document(s) · "+session.evidence_reviewed_count+" reviewed evidence item(s) · "+session.processing_state.replaceAll("_"," ");const actions=document.createElement("div");actions.className="actions";const open=document.createElement("button");open.className="secondary";open.textContent="Continue this workspace";open.addEventListener("click",async()=>{{open.disabled=true;try{{await participantResumeSession(session.intake_session_id);target.replaceChildren();}}catch(error){{participantStatus(error.message,true);open.disabled=false;}}}});const remove=document.createElement("button");remove.className="danger";remove.textContent="Delete workspace";remove.addEventListener("click",async()=>{{if(!window.confirm("Permanently delete this protected workspace and its saved documents? This cannot be undone."))return;remove.disabled=true;try{{await participantRequest("/api/withdraw",{{intake_session_id:session.intake_session_id,reason:"participant requested deletion of saved workspace",delete_now:true}});if(participantSessionId===session.intake_session_id){{participantSessionId="";document.getElementById("participantDocuments").innerHTML="<p class='quiet'>No workspace selected.</p>";document.getElementById("participantArtifactResults").innerHTML="<p class='quiet'>Select a document to prepare its evidence.</p>";}}row.remove();participantStatus("The selected workspace and its protected content were deleted.");if(!target.children.length)target.textContent="No active saved workspaces were found for this participant account.";}}catch(error){{participantStatus(error.message,true);remove.disabled=false;}}}});actions.append(open,remove);row.append(title,detail,actions);target.append(row);}});}}catch(error){{participantStatus(error.message,true);}}}}
document.getElementById("participantLoadSessions").addEventListener("click",participantLoadSessions);
function participantArtifactCard(artifact){{const box=document.createElement("div");box.className="result";const title=document.createElement("strong");title.textContent=artifact.original_filename||artifact.source_artifact_id;const detail=document.createElement("div");detail.className="quiet";detail.textContent=artifact.source_artifact_id+" · "+artifact.document_type.replaceAll("_"," ");const extract=document.createElement("button");extract.className="secondary";extract.textContent="Prepare evidence review";const reviewBox=document.createElement("div");extract.addEventListener("click",async()=>{{extract.disabled=true;extract.textContent="Preparing evidence review...";try{{const result=await participantRequest("/api/extractions",{{intake_session_id:participantSessionId,source_artifact_id:artifact.source_artifact_id}});extract.textContent="Evidence review ready";reviewBox.replaceChildren();const candidates=Array.isArray(result.evidence_candidates)?result.evidence_candidates:[];const renderedEvidenceIds=new Set();if(!candidates.length){{const empty=document.createElement("div");empty.className="status";empty.textContent="No reviewable evidence was extracted from this document.";reviewBox.append(empty);}}candidates.forEach(candidate=>{{if(renderedEvidenceIds.has(candidate.evidence_id))return;renderedEvidenceIds.add(candidate.evidence_id);const item=document.createElement("div");item.className="candidate";const text=document.createElement("p");text.textContent=candidate.evidence_text;const actions=document.createElement("div");actions.className="actions";[["Keep this evidence","accepted"],["Exclude","rejected"]].forEach(([label,disposition])=>{{const action=document.createElement("button");action.textContent=label;action.className=disposition==="accepted"?"":"secondary";action.addEventListener("click",async()=>{{action.disabled=true;try{{await participantRequest("/api/evidence/review",{{intake_session_id:participantSessionId,evidence_id:candidate.evidence_id,disposition,corrected_text:"",reason:"participant review"}});item.className="candidate reviewed";actions.innerHTML="";const saved=document.createElement("div");saved.className="status";saved.textContent=disposition==="accepted"?"Included in your evidence review.":"Excluded from downstream use.";item.append(saved);const change=document.createElement("button");change.className="secondary";change.textContent="Change decision";change.addEventListener("click",()=>{{item.className="candidate";saved.remove();change.remove();actions.innerHTML="";[["Keep this evidence","accepted"],["Exclude","rejected"]].forEach(([againLabel,againDisposition])=>{{const again=document.createElement("button");again.textContent=againLabel;again.className=againDisposition==="accepted"?"":"secondary";again.addEventListener("click",()=>{{participantRequest("/api/evidence/review",{{intake_session_id:participantSessionId,evidence_id:candidate.evidence_id,disposition:againDisposition,corrected_text:"",reason:"participant review"}}).then(()=>{{item.className="candidate reviewed";actions.innerHTML="";const updated=document.createElement("div");updated.className="status";updated.textContent=againDisposition==="accepted"?"Included in your evidence review.":"Excluded from downstream use.";item.append(updated);}}).catch(error=>participantStatus(error.message,true));}});actions.append(again);}});}});actions.append(change);}}catch(error){{action.disabled=false;participantStatus(error.message,true);}}}});actions.append(action);}});item.append(text,actions);reviewBox.append(item);}});}}catch(error){{participantStatus(error.message,true);extract.disabled=false;extract.textContent="Prepare evidence review";}}}});box.append(title,detail,extract,reviewBox);document.getElementById("participantArtifactResults").prepend(box);}}
function participantArtifactCard(artifact,savedExtraction=null){{const documents=document.getElementById("participantDocuments");if(documents.querySelector(".quiet"))documents.replaceChildren();const documentButton=document.createElement("button");documentButton.className="participant-document";documentButton.textContent=artifact.original_filename||artifact.source_artifact_id;const documentDetail=document.createElement("small");documentDetail.textContent=artifact.document_type.replaceAll("_"," ");documentButton.append(documentDetail);let restored=savedExtraction;documentButton.addEventListener("click",async()=>{{document.querySelectorAll(".participant-document").forEach(item=>item.classList.remove("active"));documentButton.classList.add("active");const list=document.getElementById("participantArtifactResults");list.innerHTML="<p class='quiet'>Preparing evidence review...</p>";try{{const extraction=restored||await participantRequest("/api/extractions",{{intake_session_id:participantSessionId,source_artifact_id:artifact.source_artifact_id}});restored=null;participantRenderStatements(extraction.evidence_candidates||[]);}}catch(error){{list.innerHTML="";participantStatus(error.message,true);}}}});documents.append(documentButton);}}
function participantRenderStatements(candidates){{const list=document.getElementById("participantArtifactResults");list.replaceChildren();if(!candidates.length){{list.innerHTML="<p class='quiet'>No reviewable evidence was extracted from this document.</p>";return;}}const seen=new Set();candidates.forEach(candidate=>{{if(seen.has(candidate.evidence_id))return;seen.add(candidate.evidence_id);const row=document.createElement("button");row.className="candidate participant-statement";const text=document.createElement("p");text.textContent=candidate.evidence_text;const state=document.createElement("small");const disposition=candidate.current_review_disposition||(candidate.included_in_downstream?"accepted":candidate.review_status==="unreviewed"?"":"rejected");const setState=value=>{{candidate.current_review_disposition=value;row.className="candidate participant-statement"+(value?" reviewed":"");state.textContent=value==="accepted"||value==="corrected"?"Included in your evidence review.":value==="rejected"?"Excluded from downstream use.":"Needs your decision";}};setState(disposition);row.append(text,state);row.addEventListener("click",()=>{{document.querySelectorAll(".participant-statement").forEach(item=>item.classList.remove("active"));row.classList.add("active");participantSelectEvidence(candidate,setState);}});list.append(row);}});}}
function participantSelectEvidence(candidate,setRowState){{const panel=document.getElementById("participantSelectedEvidence");panel.replaceChildren();const text=document.createElement("p");text.textContent=candidate.evidence_text;const source=document.createElement("p");source.className="quiet";source.textContent=candidate.source_locator||"Source location retained";const actions=document.createElement("div");actions.className="actions";const status=document.createElement("div");status.className="status";const decide=async disposition=>{{try{{await participantRequest("/api/evidence/review",{{intake_session_id:participantSessionId,evidence_id:candidate.evidence_id,disposition,corrected_text:"",reason:"participant review"}});setRowState(disposition);status.textContent=disposition==="accepted"?"Included in your evidence review.":"Excluded from downstream use.";}}catch(error){{status.textContent=error.message;status.className="status error";}}}};[["Keep this evidence","accepted",""],["Exclude","rejected","secondary"]].forEach(([label,disposition,klass])=>{{const button=document.createElement("button");button.textContent=label;button.className=klass;button.addEventListener("click",()=>decide(disposition));actions.append(button);}});panel.append(text,source,actions,status);}}
...
document.getElementById("participantReport").addEventListener("click",async()=>{{const status=document.getElementById("participantReportStatus");if(participantProcessingState==="review_complete"){{status.textContent="Checking the governed report stage...";try{{const result=await participantRequest("/api/outputs/preview",{{intake_session_id:participantSessionId}});const preview=result.participant_preview||{{}};status.replaceChildren();const heading=document.createElement("strong");heading.textContent=preview.title||"Working capability overview";status.append(heading);(preview.interpretations||[]).forEach(item=>{{const card=document.createElement("div");card.className="candidate";const title=document.createElement("strong");title.textContent=item.capability;const summary=document.createElement("p");summary.textContent=item.participant_summary;const boundary=document.createElement("p");boundary.className="quiet";boundary.textContent="Boundary: "+(item.negative_boundaries||[]).join("; ");card.append(title,summary,boundary);status.append(card)}});if(!(preview.interpretations||[]).length)status.append(document.createTextNode("The report is held for output assurance."));}}catch(error){{status.textContent="Your evidence review is complete, but capability mapping and independent acceptance are still required before a participant report can be produced. No graph submission has occurred.";}}return}}const reviewed=[...document.querySelectorAll("#participantArtifactResults .candidate.reviewed")].filter(item=>item.textContent.includes("Included in your evidence review."));if(!reviewed.length){{status.textContent="No evidence has been kept yet. Review and keep at least one source statement first.";return;}}status.innerHTML="";const heading=document.createElement("strong");heading.textContent="Your reviewed evidence summary";const note=document.createElement("p");note.textContent="These are the source statements you chose to keep. Submit them for authorized evidence review when ready.";status.append(heading,note);const list=document.createElement("ul");reviewed.forEach(item=>{{const entry=document.createElement("li");const text=item.querySelector("p");entry.textContent=text?text.textContent:"Reviewed evidence";list.append(entry);}});status.append(list);const submit=document.getElementById("participantSubmit");submit.disabled=false;document.getElementById("participantSubmitStatus").textContent="Your evidence summary is ready. Submit it when you are finished making evidence decisions.";}});
document.getElementById("participantSubmit").addEventListener("click",async()=>{{if(!participantSessionId)return;if(!window.confirm("Submit your reviewed evidence for authorized review?"))return;const button=document.getElementById("participantSubmit");button.disabled=true;try{{const result=await participantRequest("/api/sessions/submit-review",{{intake_session_id:participantSessionId}});document.getElementById("participantSubmitStatus").textContent=`Submitted for authorized review. ${{result.included_evidence_count}} evidence item(s) will be considered. You may now sign out and return later; withdrawal remains available.`;document.getElementById("participantReport").disabled=true;document.getElementById("participantChoose").disabled=true;document.getElementById("participantStage").disabled=true;document.querySelectorAll("#participantArtifactResults button").forEach(control=>control.disabled=true);}}catch(error){{document.getElementById("participantSubmitStatus").textContent=error.message;button.disabled=false;}}}});
    ...
    *,
    ...
    for extraction in session.get("evidence_extractions", []):
        ...
        for candidate in extraction.get("evidence_candidates", []):
            ...
            evidence_key = evidence_text.casefold()
...
.review-shell{{width:min(1440px,calc(100% - 32px));}}.review-header{{display:flex;justify-content:space-between;align-items:start;gap:18px}}.review-grid{{display:grid;grid-template-columns:250px minmax(360px,1fr) 360px;gap:16px;margin-top:20px}}.pane{{border:1px solid var(--line);border-radius:16px;background:#fff;padding:16px;min-height:520px}}.pane h2{{font:700 1.15rem/1.2 Inter,sans-serif}}.document-row,.statement-row{{width:100%;text-align:left;color:var(--ink);background:#fff;border:1px solid var(--line);border-radius:10px;margin:8px 0;padding:12px;cursor:pointer;overflow:hidden;overflow-wrap:anywhere}}.statement-row{{max-height:96px}}.document-row.active,.statement-row.active{{border-color:var(--sage);background:var(--pale)}}.document-row small,.statement-row small{{display:block;color:var(--muted);margin-top:5px}}.decision-row{{display:flex;gap:8px;flex-wrap:wrap}}.decision-row button{{margin:0}}.workspace-bar{{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}}.workspace-bar input,.workspace-bar select{{width:auto;flex:1 1 170px}}.review-queue-head{{display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap}}.review-queue-head h2{{margin-bottom:4px}}.queue-item{{display:grid;grid-template-columns:minmax(220px,1fr) minmax(280px,2fr) auto;align-items:center;gap:14px;margin-top:10px;padding:12px;border:1px solid var(--line);border-radius:12px;background:#fff}}.queue-item.awaiting{{border-left:5px solid var(--sage);background:var(--pale)}}.queue-item button{{margin:0}}.summary-strip{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:16px}}.metric{{padding:13px;border:1px solid var(--line);border-radius:12px;background:#fff}}.metric strong{{display:block;font-size:1.3rem;color:var(--deep)}}.finish-review{{display:flex;align-items:center;justify-content:space-between;gap:20px;margin-top:16px}}.finish-review button{{white-space:nowrap}}@media(max-width:950px){{.review-grid{{grid-template-columns:1fr}}.pane{{min-height:0}}.summary-strip{{grid-template-columns:repeat(2,1fr)}}}}@media(max-width:700px){{.queue-item{{grid-template-columns:1fr}}.finish-review{{display:block}}}}</style></head>
<style>.review-grid>*{{min-width:0}}.pane{{min-width:0;overflow:hidden}}.pane p{{max-width:100%;overflow-wrap:anywhere;word-break:break-word}}</style>
...
<section class="notice"><strong>Protection controls are active.</strong><div class="controls"><div class="control"><strong>Encrypted at rest</strong><span>Protected with per-session keys.</span></div><div class="control"><strong>Inspected in memory</strong><span>Document bytes are checked before encrypted storage.</span></div><div class="control"><strong>Finite lifecycle</strong><span>Withdrawal and retention controls remain executable.</span></div></div></section>
<section class="card"><div class="review-queue-head"><div><div class="eyebrow">Review queue</div><h2>Continue participant-submitted work.</h2><p class="quiet">Sessions awaiting review appear first. Open one to review its source-grounded statements.</p></div><button id="loadSessions" class="secondary">Refresh review queue</button></div><div id="sessions"><p class="quiet">Loading active participant sessions…</p></div></section>
...
<section class="card"><div class="eyebrow">Open workspace</div><div class="workspace-bar"><input id="file" type="file" accept=".pdf,.doc,.docx,.rtf,.txt,.csv,.zip" disabled><select id="documentType" disabled><option value="">Document type</option><option value="professional_profile">Professional profile</option><option value="career_document">Career document</option><option value="credential_learning">Credential or learning</option><option value="supporting_evidence">Supporting evidence</option></select><button id="stage" disabled>Add evidence</button></div><div id="workspaceStatus" class="status">Choose a session from the review queue to begin.</div></section>
...
<section class="card finish-review"><div><div class="eyebrow">Finish evidence review</div><h2>Send accepted evidence forward.</h2><p class="quiet">Completing review returns the evidence result to the participant and opens the capability step below. It does not write to the graph.</p><div id="completionStatus" class="status">Open a participant-submitted workspace and review every statement.</div></div><div class="actions"><button id="completeReview">Complete evidence review</button><button id="continueMapping" class="secondary" type="button">Prepare capability step</button></div></section>
<section class="card"><div class="eyebrow">Missing capability term</div><h2>Add a bounded capability name.</h2><p class="quiet">Use this only when no listed capability fits. The name must be concise and share meaningful, non-generic language with the accepted evidence. It remains session-scoped and does not silently expand the canonical vocabulary. Training remains preparation or knowledge exposure, not proof of operational performance.</p><label for="directCustomCapability">New capability name (optional)</label><input id="directCustomCapability" maxlength="100" placeholder="For example, Project Portfolio Coordination"><div id="customCapabilityStatus" class="status">Leave blank to use a listed capability.</div></section>
<script>addEventListener("DOMContentLoaded",()=>{{const input=document.getElementById("directCustomCapability"),select=document.getElementById("directCapability"),status=document.getElementById("customCapabilityStatus");input.addEventListener("input",()=>{{const old=select.querySelector('option[data-custom="true"]');if(old)old.remove();const name=input.value.trim();if(!name){{status.textContent="Leave blank to use a listed capability.";return}}const option=new Option("New: "+name,"CUSTOM:"+name);option.dataset.custom="true";select.add(option);select.value=option.value;status.textContent="This new term will be checked against the accepted evidence before it can be recorded.";}});document.getElementById("previewCapabilityReport").addEventListener("click",async event=>{{event.stopImmediatePropagation();const target=document.getElementById("capabilityStatus");target.textContent="Preparing the bounded participant report...";try{{const result=await request("/api/outputs/preview",{{intake_session_id:sessionId}}),preview=result.participant_preview||{{}};target.replaceChildren();const heading=document.createElement("strong");heading.textContent=preview.title||"Working capability overview";target.append(heading);(preview.interpretations||[]).forEach(item=>{{const card=document.createElement("div");card.className="candidate";const title=document.createElement("strong");title.textContent=item.capability;const summary=document.createElement("p");summary.textContent=item.participant_summary+" "+item.inference_level.replaceAll("_"," ")+" · confidence "+item.confidence+" · "+item.mapping_ids.length+" supporting mapping(s).";const boundary=document.createElement("p");boundary.className="quiet";boundary.textContent="Boundary: "+(item.negative_boundaries||[]).join("; ");card.append(title,summary,boundary);target.append(card)}});const limit=document.createElement("p");limit.className="quiet";limit.textContent=preview.limits||"No graph write occurred.";target.append(limit);}}catch(error){{target.textContent=error.message}}}},true);}});</script>
<section id="capabilityStep" class="card"><div class="eyebrow">Capability and report</div><h2>Link accepted evidence to one bounded capability.</h2><p class="quiet">This is a direct authorized reviewer decision. It preserves the evidence link, rationale, boundary, confidence rule, and audit history without requiring a second reviewer.</p><div class="grid"><div><label for="directEvidence">Accepted evidence</label><select id="directEvidence"><option value="">Choose accepted evidence</option></select><label for="directCapability">Capability</label><select id="directCapability"><option value="">Choose capability</option></select><label for="directInference">Strength of support</label><select id="directInference"><option value="directly_demonstrated">Directly demonstrated</option><option value="strongly_inferred">Strongly inferred</option><option value="contextually_suggested">Contextually suggested</option></select></div><div><label for="directRationale">Why this evidence supports the capability</label><textarea id="directRationale" maxlength="2000"></textarea><label for="directBoundary">What this does not establish</label><textarea id="directBoundary" maxlength="2000">This does not establish overall competence, formal authority, outcome quality, or a durable trait.</textarea></div></div><div class="actions"><button id="acceptCapability">Accept capability link</button><button id="previewCapabilityReport" class="secondary">Prepare participant report</button></div><div id="capabilityStatus" class="status">Complete evidence review, then prepare this step.</div></section>
<script>addEventListener("DOMContentLoaded",()=>{{const button=document.getElementById("completeReview"),message=document.getElementById("completionStatus"),capability=document.getElementById("capabilityStep");async function prepareCapability(){{if(!workspace||!sessionId){{document.getElementById("capabilityStatus").textContent="Open a participant workspace first.";return}}capability.scrollIntoView({{behavior:"smooth",block:"start"}});const evidence=document.getElementById("directEvidence");evidence.innerHTML='<option value="">Choose accepted evidence</option>';const accepted=(workspace.evidence_extractions||[]).flatMap(x=>x.evidence_candidates||[]).filter(x=>x.current_review_disposition==="accepted"||x.current_review_disposition==="corrected");accepted.forEach(item=>evidence.add(new Option(item.evidence_text.slice(0,120),item.evidence_id)));const select=document.getElementById("directCapability");if(select.options.length===1){{try{{const result=await request("/api/mappings/vocabulary",{{}});result.capabilities.forEach(item=>select.add(new Option(item.capability_name,item.profile_capability_id)))}}catch(error){{document.getElementById("capabilityStatus").textContent=error.message}}}}document.getElementById("capabilityStatus").textContent=accepted.length?"Choose accepted evidence and the bounded capability it supports.":"No accepted evidence is available for capability linking."}}document.getElementById("continueMapping").addEventListener("click",prepareCapability);button.addEventListener("click",async()=>{{if(!sessionId){{message.textContent="Choose a participant-submitted workspace first.";return}}if(!confirm("Complete this evidence review and return the accepted result to the participant?"))return;button.disabled=true;try{{const result=await request("/api/sessions/complete-review",{{intake_session_id:sessionId}});workspace.session.processing_state=result.processing_state;message.textContent=`Review completed and returned to the participant. ${{result.accepted_evidence_count}} accepted evidence item(s) are ready for the capability step. No graph write occurred.`;status("Evidence review completed. Continue with the capability step below.");byId("loadSessions").click();await prepareCapability();}}catch(error){{message.textContent=error.message;button.disabled=false;}}}});document.getElementById("acceptCapability").addEventListener("click",async()=>{{const target=document.getElementById("capabilityStatus");try{{const result=await request("/api/mappings/accept-direct",{{intake_session_id:sessionId,evidence_id:document.getElementById("directEvidence").value,profile_capability_id:document.getElementById("directCapability").value,inference_level:document.getElementById("directInference").value,rationale:document.getElementById("directRationale").value,negative_boundary:document.getElementById("directBoundary").value}});target.textContent=`Accepted ${{result.mapping.capability_name}}. The evidence link and boundary are recorded; the participant report can now be prepared.`;}}catch(error){{target.textContent=error.message}}}});document.getElementById("previewCapabilityReport").addEventListener("click",async()=>{{const target=document.getElementById("capabilityStatus");try{{const result=await request("/api/outputs/preview",{{intake_session_id:sessionId}});target.textContent=`Participant report ready: ${{result.participant_preview.interpretations.length}} bounded capability interpretation(s). No graph write occurred.`;}}catch(error){{target.textContent=error.message}}}})}});</script>
</main><script>const CSRF={csrf};const CAN_DELETE={str(auth.role == "owner").lower()};let sessionId="",workspace=null,selectedArtifact="",selectedCandidate=null;const byId=id=>document.getElementById(id);async function request(path,body=null,method="POST"){{const options={{method,headers:{{"X-PIA-CSRF":CSRF}}}};if(body!==null){{options.headers["Content-Type"]="application/json";options.body=JSON.stringify(body)}}const response=await fetch(path,options);const data=await response.json();if(!response.ok)throw new Error(data.error||"Request failed.");return data}}function status(message,error=false){{const el=byId("workspaceStatus");el.textContent=message;el.className="status"+(error?" error":"")}}function recount(){{const all=workspace?.evidence_extractions?.flatMap(x=>x.evidence_candidates)||[];byId("acceptedCount").textContent=all.filter(x=>x.current_review_disposition==="accepted").length;byId("editedCount").textContent=all.filter(x=>x.current_review_disposition==="corrected").length;byId("rejectedCount").textContent=all.filter(x=>x.current_review_disposition==="rejected").length;byId("pendingCount").textContent=all.filter(x=>!x.current_review_disposition).length}}function render(){{const docs=byId("documents");docs.replaceChildren();const artifactMap=new Map((workspace?.artifacts||[]).map(x=>[x.source_artifact_id,x]));workspace?.artifacts?.forEach(a=>{{const b=document.createElement("button");b.className="document-row"+(a.source_artifact_id===selectedArtifact?" active":"");b.textContent=a.original_filename;const s=document.createElement("small");s.textContent=a.document_type.replaceAll("_"," ");b.append(s);b.onclick=()=>{{selectedArtifact=a.source_artifact_id;render()}};docs.append(b)}});const list=byId("statements");list.replaceChildren();const extraction=workspace?.evidence_extractions?.find(x=>x.source_artifact_id===selectedArtifact);if(!extraction){{list.innerHTML="<p class='quiet'>Select a document with extracted statements.</p>"}}else extraction.evidence_candidates.forEach(c=>{{const b=document.createElement("button");b.className="statement-row"+(c.evidence_id===selectedCandidate?.evidence_id?" active":"");b.textContent=c.evidence_text;const s=document.createElement("small");s.textContent=c.source_locator+" · "+(c.current_review_disposition||"Needs review");b.append(s);b.onclick=()=>{{selectedCandidate=c;render()}};list.append(b)}});const panel=byId("selection");panel.replaceChildren();if(selectedCandidate){{const text=document.createElement("p");text.textContent=selectedCandidate.evidence_text;const source=document.createElement("p");source.className="quiet";source.textContent=selectedCandidate.source_locator;const controls=document.createElement("div");controls.className="decision-row";[["Accept","accepted",""],["Edit","corrected","secondary"],["Reject","rejected","danger"]].forEach(([label,decision,klass])=>{{const button=document.createElement("button");button.textContent=label;button.className=klass;button.onclick=async()=>{{let corrected="";if(decision==="corrected"){{corrected=prompt("Correct the source-grounded wording:",selectedCandidate.evidence_text)||"";if(!corrected)return}}try{{const result=await request("/api/evidence/review",{{intake_session_id:sessionId,evidence_id:selectedCandidate.evidence_id,disposition:decision,corrected_text:corrected,reason:"reviewer decision"}});Object.assign(selectedCandidate,result.evidence_candidate);status("Decision recorded in the protected audit history.");render();recount()}}catch(error){{status(error.message,true)}}}};controls.append(button)}});panel.append(text,source,controls)}}recount()}}async function openWorkspace(id){{try{{workspace=await request("/api/sessions/resume",{{intake_session_id:id}});sessionId=workspace.session.intake_session_id;selectedArtifact=workspace.artifacts?.[0]?.source_artifact_id||"";selectedCandidate=null;byId("file").disabled=false;byId("documentType").disabled=false;byId("stage").disabled=false;status("Protected workspace open: "+sessionId);render()}}catch(error){{status(error.message,true)}}}}byId("loadSessions").onclick=async()=>{{const button=byId("loadSessions");button.disabled=true;try{{const result=await request("/api/sessions",null,"GET");const target=byId("sessions");target.replaceChildren();const ordered=[...result.sessions].sort((a,b)=>(b.processing_state==="awaiting_review")-(a.processing_state==="awaiting_review"));if(!ordered.length)target.innerHTML="<p class='quiet'>No active participant sessions are available.</p>";ordered.forEach(item=>{{const row=document.createElement("div");row.className="queue-item"+(item.processing_state==="awaiting_review"?" awaiting":"");const title=document.createElement("strong");title.textContent=item.participant_label;const detail=document.createElement("div");detail.className="quiet";detail.textContent=item.processing_state.replaceAll("_"," ")+" · "+item.artifact_count+" document(s) · "+item.evidence_reviewed_count+" reviewed · "+item.evidence_pending_count+" pending";const actions=document.createElement("div");actions.className="actions";const open=document.createElement("button");open.className="secondary";open.textContent=item.processing_state==="awaiting_review"?"Review this submission":"Continue this session";open.onclick=()=>openWorkspace(item.intake_session_id);actions.append(open);if(CAN_DELETE){{const remove=document.createElement("button");remove.className="danger";remove.textContent="Delete session";remove.onclick=async()=>{{if(!confirm("Permanently delete this participant session and its protected content? This cannot be undone."))return;remove.disabled=true;try{{await request("/api/withdraw",{{intake_session_id:item.intake_session_id,reason:"owner removed erroneous or obsolete session",delete_now:true}});if(sessionId===item.intake_session_id){{sessionId="";workspace=null;selectedArtifact="";selectedCandidate=null;byId("documents").innerHTML="<p class='quiet'>No workspace selected.</p>";byId("statements").innerHTML="<p class='quiet'>Select a document to review its extracted statements.</p>";byId("selection").innerHTML="<p class='quiet'>Select a statement to accept, edit, or reject it.</p>";}}row.remove();status("The selected participant session was permanently deleted by the owner.");}}catch(error){{status(error.message,true);remove.disabled=false;}}}};actions.append(remove);}}row.append(title,detail,actions);target.append(row)}})}}catch(error){{status(error.message,true)}}finally{{button.disabled=false}}}};byId("stage").onclick=async()=>{{const file=byId("file").files[0],type=byId("documentType").value;if(!sessionId||!file||!type){{status("Choose a document and its type first.",true);return}}try{{const bytes=new Uint8Array(await file.arrayBuffer());let binary="";for(let i=0;i<bytes.length;i+=0x8000)binary+=String.fromCharCode(...bytes.subarray(i,i+0x8000));const artifact=await request("/api/artifacts",{{intake_session_id:sessionId,original_filename:file.name,document_type:type,content_base64:btoa(binary)}});await request("/api/extractions",{{intake_session_id:sessionId,source_artifact_id:artifact.source_artifact_id}});await openWorkspace(sessionId);status("Document added and statements prepared for review.")}}catch(error){{status(error.message,true)}}}};byId("logout").onclick=async()=>{{await request("/api/logout",{{}});location.reload()}};byId("loadSessions").click();</script></body></html>"""
...
function sessionProgressLabel(item){{
  if(item.processing_state==="awaiting_review")return "Submitted for reviewer work";
        ...
        def do_GET(self) -> None:
            ...
            if path in {"/", "/technical"}:
                ...
                if auth is None:
                    ...
                    page = _main_page(
                        auth,
                        external_lookup_configured=(
                            credential_linkage is not None
                        ),
                    )
                else:
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                if path == "/api/sessions/resume":
                    ...
                    return
                if path == "/api/sessions/submit-review":
                    unexpected = sorted(set(body) - {"intake_session_id"})
                    if unexpected:
                        raise IntakePreflightError(
                            "Review-submission fields are not permitted: "
                            + ", ".join(unexpected)
                        )
                    self._json(
                        HTTPStatus.OK,
                    ...
                    return
                if path == "/api/sessions/complete-review":
                    unexpected = sorted(set(body) - {"intake_session_id"})
                    if unexpected:
                        raise IntakePreflightError(
                            "Review-completion fields are not permitted: "
                            + ", ".join(unexpected)
                        )
                    self._json(
                        HTTPStatus.OK,
                        store.complete_evidence_review(
                            str(body.get("intake_session_id", "")),
                            actor_subject=auth.subject,
                            actor_role=auth.role,
                        ),
                ...
                if path == "/api/evidence/review":
                    ...
                    return
                if path == "/api/mappings/accept-direct":
                    allowed = {
                        "intake_session_id",
                        "evidence_id",
                    ...
                    if unexpected:
                        raise IntakePreflightError(
                            "Direct capability-decision fields are not permitted: "
                            + ", ".join(unexpected)
                        )
                    inference = str(
                        body.get("inference_level", "")
                    ).strip()
                    confidence = {
                        "directly_demonstrated": 0.8,
                        "strongly_inferred": 0.7,
                        "contextually_suggested": 0.4,
                    }.get(inference)
                    if confidence is None:
                        raise IntakePreflightError(
                            "Choose a valid strength of support."
                        )
                    rationale = str(body.get("rationale", "")).strip()
                    boundary = str(
                        body.get("negative_boundary", "")
                    ).strip()
                    if len(rationale) < 16 or len(boundary) < 16:
                        raise IntakePreflightError(
                            "Provide a clear rationale and a meaningful boundary."
                        )
                    profile_capability_id = str(
                        body.get("profile_capability_id", "")
                    ).strip()
                    custom_capability_name = str(
                        body.get("custom_capability_name", "")
                    ).strip()
                    if profile_capability_id.startswith("CUSTOM:"):
                        custom_capability_name = profile_capability_id[7:]
                        profile_capability_id = ""
                    self._json(
                        HTTPStatus.CREATED,
                        mapping.accept_direct(
                            session_id=str(
                                body.get("intake_session_id", "")
                            ),
                            evidence_id=str(body.get("evidence_id", "")),
                            proposal={
                                ...
                                "alignment_basis": rationale,
                                "behavioral_basis": rationale,
                                "negative_boundary": boundary,
                                "scope_limit": (
                                    "Limited to the behavior described in the "
                                    "accepted source statement and its stated context."
                                ),
                                "source_independence_note": (
                                    "Based on this accepted evidence item; independent "
                                    "corroboration is not asserted."
                                ),
                            ...
                            actor_subject=auth.subject,
                            actor_role=auth.role,
                            custom_capability_name=custom_capability_name,
                        ),
                    )
                    return

...
def normalize_candidates(blocks: list[SourceBlock]) -> list[dict[str, Any]]:
    ...
    for block in blocks:
        ...
        for chunk in _candidate_chunks(block.text):
            fingerprint = re.sub(r"\s+", " ", chunk).strip().casefold()
            existing = candidates_by_text.get(fingerprint)
            if existing is not None:
                duplicate_locations = existing["duplicate_source_locators"]
                if block.source_locator not in duplicate_locations:
                    duplicate_locations.append(block.source_locator)
                existing["source_occurrence_count"] = 1 + len(
                    duplicate_locations
                )
                existing["deduplication_status"] = "exact_text_collapsed"
                continue
            ...
            candidates_by_text[fingerprint] = candidate
    return candidates

// PIA Reference meta-ontology registry v0.1
//
...
//           managed_by = '002_pia_reference_meta_ontology' through a reviewed
//           recovery migration.
//
// This migration uses the OSI/PIA shared governance vocabulary while
// registering only PIA Reference concepts and relationships. It does not
// relabel, rewrite, or delete existing PIA domain data.

CREATE CONSTRAINT pia_meta_ontology_id_unique IF NOT EXISTS
FOR (n:Ontology)
REQUIRE n.ontology_id IS UNIQUE;

CREATE CONSTRAINT pia_meta_concept_id_unique IF NOT EXISTS
FOR (n:Concept)
REQUIRE n.concept_id IS UNIQUE;

CREATE CONSTRAINT pia_meta_relationship_definition_id_unique IF NOT EXISTS
FOR (n:RelationshipDefinition)
REQUIRE n.relationship_definition_id IS UNIQUE;

CREATE CONSTRAINT pia_meta_lifecycle_state_id_unique IF NOT EXISTS
FOR (n:LifecycleState)
REQUIRE n.lifecycle_state_id IS UNIQUE;

CREATE CONSTRAINT pia_meta_knowledge_state_id_unique IF NOT EXISTS
FOR (n:KnowledgeState)
REQUIRE n.knowledge_state_id IS UNIQUE;

CREATE CONSTRAINT pia_meta_confidence_model_id_unique IF NOT EXISTS
FOR (n:ConfidenceModel)
REQUIRE n.confidence_model_id IS UNIQUE;

MERGE (ontology:Ontology {ontology_id: 'ontology:pia-reference'})
ON CREATE SET
  ontology.name = 'PIA Reference Ontology',
  ontology.version = '0.1.0',
  ontology.scope = 'pia-reference',
  ...
  ontology.description =
    'Working registry for the concepts, relationships, lifecycle, knowledge state, and confidence vocabulary observed in the PIA Reference graph.',
  ontology.source_ref = 'ontology/META_ONTOLOGY.md',
  ontology.managed_by = '002_pia_reference_meta_ontology',
...
CALL (ontology) {
  ...
  ON CREATE SET
    ...
    state.managed_by = '002_pia_reference_meta_ontology',
...
CALL (ontology) {
  MERGE (model:ConfidenceModel {
    confidence_model_id: 'confidence:pia-reference:qualitative-v1'
  })
  ON CREATE SET
    model.name = 'PIA Reference Qualitative Confidence',
    model.version = '1.0',
    ...
    model.description =
      'Documents the shared qualitative confidence vocabulary for PIA interpretations without converting confidence into a person score.',
    model.managed_by = '002_pia_reference_meta_ontology',
...
CALL (ontology) {
  MERGE (concept:Concept {
    concept_id: 'concept:pia-reference:Concept'
  })
  ON CREATE SET
    ...
    concept.managed_by = '002_pia_reference_meta_ontology',
...
CALL (ontology) {
  MERGE (definition:RelationshipDefinition {
    relationship_definition_id:
      'relationship-definition:pia-reference:DEFINES_RELATIONSHIP'
  })
  ON CREATE SET
    ...
    definition.managed_by = '002_pia_reference_meta_ontology',
  ...
  UNWIND [
    {
      ...
      stable_identity_property: 'identity_id'
    },
    {
      graph_label: 'Observation',
      stable_identity_property: 'observation_id'
    },
    {
      graph_label: 'Representation',
      stable_identity_property: 'representation_id'
    }
  ] AS schema_concept
  MERGE (concept:Concept {
    concept_id: 'concept:pia-reference:' + schema_concept.graph_label
  })
  ON CREATE SET
    concept.name = schema_concept.graph_label,
    concept.graph_label = schema_concept.graph_label,
    concept.item_kind = 'graph_label',
    concept.scope = 'pia-reference',
    concept.stable_identity_property =
      schema_concept.stable_identity_property,
    concept.knowledge_lifecycle_state = 'formulation',
    concept.ontology_status = 'working',
    concept.implementation_status = 'implemented',
    concept.registration_basis = 'schema_constraint_without_instance',
    concept.observed_node_count_at_registration = 0,
    concept.source_ref = 'ontology/META_ONTOLOGY.md',
    concept.managed_by = '002_pia_reference_meta_ontology',
    concept.created_at = datetime()
  MERGE (ontology)-[:DEFINES_CONCEPT]->(concept)
  RETURN count(concept) AS schema_concept_count
}
...
CALL (ontology) {
  ...
  MERGE (definition:RelationshipDefinition {
    relationship_definition_id:
      'relationship-definition:pia-reference:' + relationship_type
  })
  ON CREATE SET

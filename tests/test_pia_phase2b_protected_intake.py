...
from software.intake.evidence_intake_linkage import ProtectedEvidenceIntakeLinkage
...
class PIAPhase2BProtectedIntakeTests(unittest.TestCase):
    ...
    def test_participant_submits_completed_evidence_for_review(self) -> None:
        session = self.store.create_session(
            participant_label="Synthetic participant workspace",
            purpose="Review represented professional evidence",
            processing_scope=(
                "evidence_extraction|participant_report|"
                "capability_mapping|report_generation"
            ),
            consent_status="granted",
            confidentiality="participant_private",
            retention_class="14_days",
            actor_subject="participant-001",
            actor_role="participant",
        )
        session_id = str(session["intake_session_id"])
        artifact = self.store.stage_upload(
            session_id=session_id,
            original_filename="synthetic-evidence.txt",
            content=(
                b"Coordinated a documented operational handoff and created "
                b"repeatable instructions for the receiving team."
            ),
            document_type="supporting_evidence",
            actor_subject="participant-001",
            actor_role="participant",
        )
        linkage = ProtectedEvidenceIntakeLinkage(self.store)
        extraction = linkage.extract(
            session_id=session_id,
            source_artifact_id=str(artifact["source_artifact_id"]),
            actor_subject="participant-001",
            actor_role="participant",
        )
        candidates = extraction["evidence_candidates"]
        ...
        for index, candidate in enumerate(candidates):
            linkage.review(
                ...
                reason="synthetic participant review",
                actor_subject="participant-001",
                actor_role="participant",
            )
        result = self.store.submit_for_review(
            session_id,
            actor_subject="participant-001",
            actor_role="participant",
        )
        self.assertEqual(result["processing_state"], "awaiting_review")
        self.assertEqual(result["included_evidence_count"], 1)
        resumed = self.store.resume_session(
            session_id,
            actor_subject="reviewer-001",
            actor_role="reviewer",
        )
        self.assertEqual(resumed["processing_state"], "awaiting_review")
        self.assertIn(
            "participant_evidence_submitted_for_review",
            {event["event_type"] for event in self.store._read_audit()},
        )

    def test_reviewer_completes_evidence_handoff_without_graph_write(self) -> None:
        session = self.store.create_session(
            participant_label="Synthetic participant workspace",
            purpose="Complete authorized evidence review",
            processing_scope=(
                "evidence_extraction|participant_report|"
                "capability_mapping|report_generation"
            ),
        ...
        artifact = self.store.stage_upload(
            session_id=session_id,
            original_filename="synthetic-evidence.txt",
            content=b"Coordinated a documented operational handoff.",
            document_type="supporting_evidence",
            actor_subject="participant-001",
            actor_role="participant",
        )
        ...
        with self.assertRaises(IntakePreflightError):
            self.store.complete_evidence_review(
                session_id,
                actor_subject="reviewer-001",
                actor_role="reviewer",
            )
        for candidate in candidates:
            linkage.review(
                ...
                evidence_id=str(candidate["evidence_id"]),
                disposition="accepted",
                reason="authorized reviewer decision",
                actor_subject="reviewer-001",
                actor_role="reviewer",
            )
        result = self.store.complete_evidence_review(
            session_id,
            actor_subject="reviewer-001",
            actor_role="reviewer",
        )
        self.assertEqual(result["processing_state"], "review_complete")
        self.assertEqual(result["graph_write"], "not_performed")
        self.assertEqual(
            result["next_stage"],
            "capability_mapping_and_report_assurance",
        )
        self.assertIn(
            "participant_evidence_review_completed",
            {event["event_type"] for event in self.store._read_audit()},
        )
        mapping_result = ProtectedCapabilityMappingLinkage(
            self.store
        ).accept_direct(
            session_id=session_id,
            ...
            proposal={
                ...
                "confidence": 0.8,
                "confidence_basis": (
                    "The accepted statement directly describes a documented "
                    "operational handoff."
                ),
                "alignment_basis": (
                    "The accepted statement directly describes a documented "
                ...
                "negative_boundary": (
                    "This does not establish overall competence, authority, "
                    "outcome quality, or a durable trait."
                ),
                "scope_limit": (
                    "Limited to the behavior described in the accepted source."
                ),
                "source_independence_note": (
                    "Independent corroboration is not asserted."
                ),
        ...
        self.assertEqual(mapping_result["mapping"]["review_status"], "accepted")
        self.assertEqual(
            mapping_result["review_event"]["decision_mode"],
            "reviewer_direct_acceptance",
        )
        reusable_proposal = dict(mapping_result["mapping"])
        custom_result = ProtectedCapabilityMappingLinkage(
            self.store
        ).accept_direct(
            session_id=session_id,
            evidence_id=str(candidates[0]["evidence_id"]),
            ...
            actor_subject="reviewer-001",
            actor_role="reviewer",
            custom_capability_name="Operational Handoff Coordination",
        )
        self.assertEqual(
            custom_result["mapping"]["capability_name"],
            "Operational Handoff Coordination",
        )
        self.assertTrue(
            custom_result["mapping"]["profile_capability_id"].startswith(
                "CAP-PIA-LOCAL-"
            )
        )
        with self.assertRaises(IntakePreflightError):
            ProtectedCapabilityMappingLinkage(self.store).accept_direct(
                session_id=session_id,
                evidence_id=str(candidates[0]["evidence_id"]),
                proposal=reusable_proposal,
                actor_subject="reviewer-001",
                actor_role="reviewer",
                custom_capability_name="Burger Stand Supervision",
            )
        preview = ProtectedMappingOutputLinkage(self.store).preview(
            session_id=session_id,
            actor_role="participant",
        )
        self.assertEqual(
    ...
    def test_participant_can_list_and_resume_only_own_workspace(self) -> None:
        own = self.store.create_session(
            participant_label="Participant one's saved work",
            purpose="Continue later",
            processing_scope="evidence_extraction|participant_report",
            consent_status="granted",
            confidentiality="participant_private",
            retention_class="14_days",
            actor_subject="participant-001",
        ...
        other = self.store.create_session(
            participant_label="Another participant's work",
            purpose="Must remain isolated",
            processing_scope="evidence_extraction|participant_report",
            consent_status="granted",
            confidentiality="participant_private",
            retention_class="14_days",
            actor_subject="participant-002",
            actor_role="participant",
        )
        visible = self.store.list_resumable_sessions(
            actor_subject="participant-001",
            actor_role="participant",
        )
        self.assertEqual(
            [item["intake_session_id"] for item in visible["sessions"]],
            [own["intake_session_id"]],
        )
        resumed = self.store.resume_session(
            str(own["intake_session_id"]),
            actor_subject="participant-001",
            actor_role="participant",
        )
        self.assertEqual(resumed["participant_label"], "Participant one's saved work")
        with self.assertRaises(IntakePreflightError):
            self.store.resume_session(
                str(other["intake_session_id"]),
                actor_subject="participant-001",
                actor_role="participant",
            )
    ...
    def test_participant_can_delete_only_own_session(self) -> None:
        own = self.store.create_session(
            participant_label="Participant workspace to remove",
            purpose="Synthetic deletion control",
            processing_scope="evidence_extraction",
            consent_status="granted",
            confidentiality="participant_private",
            retention_class="14_days",
            actor_subject="participant-001",
        ...
        other = self.store.create_session(
            participant_label="Another participant workspace",
            purpose="Must remain protected",
            processing_scope="evidence_extraction",
            consent_status="granted",
            confidentiality="participant_private",
            retention_class="14_days",
        ...
        with self.assertRaises(IntakePreflightError):
            self.store.withdraw_session(
                str(other["intake_session_id"]),
                reason="Unauthorized cross-participant deletion attempt.",
                actor_subject="participant-001",
                actor_role="participant",
                delete_now=True,
            )
        deleted = self.store.withdraw_session(
            str(own["intake_session_id"]),
            reason="Participant removed erroneous workspace.",
            actor_subject="participant-001",
            actor_role="participant",
            delete_now=True,
        )
        self.assertFalse(deleted["participant_content_retained"])
        self.assertFalse(
            (self.store_root / "sessions" / str(own["intake_session_id"])).exists()
        )
        self.assertTrue(
            (self.store_root / "sessions" / str(other["intake_session_id"])).exists()
        )
    ...
    def test_protected_server_requires_login_and_csrf(self) -> None:
        ...
        with urllib.request.urlopen(protected_page_request) as response:
            page = response.read().decode()
            self.assertIn("Review evidence. Support capabilities.", page)
            self.assertIn("Protection controls are active.", page)
            self.assertIn("Continue participant-submitted work.", page)

        technical_page_request = urllib.request.Request(
            f"{base_url}/technical",
            headers={"Cookie": cookie, "User-Agent": user_agent},
        )
        with urllib.request.urlopen(technical_page_request) as response:
            technical_page = response.read().decode()
            self.assertEqual(response.status, 200)
            self.assertIn("Mapping handoff", technical_page)
            self.assertIn("Refresh review queue", page)
            self.assertIn("Extracted statements", page)
            self.assertIn("Selected statement", page)
            self.assertIn("Credential review", page)
